...
solution1 :: Int -> Point
solution1 gridNumber = let g = grid gridNumber
                           points = (,) <$> [1..298] <*> [1..298]
...
grid gridNumber = Map.fromList $ List.zip points $ List.map power points
  where points = (,) <$> [1..300] <*> [1..300]
        power (x,y) = (((x + 10) * y + gridNumber) * (x + 10) `div` 100) `rem` 10 - 5

squareSum :: Grid -> Point -> Int
squareSum g p = let points = (,) <$> [0..2] <*> [0..2]
                in List.sum $ List.map (\e -> Map.findWithDefault 0 (e <> p) g) points

solution2 :: Int -> (Int, Int, Int)
solution2 gridNumber = let g = grid gridNumber
                           sum = summedArea g
                           points = (,) <$> [1..300] <*> [1..300]
                           squares = List.concatMap (\p@(x,y) -> [ (s,p) | s <- [1.. min (300 - x) (300 - y) ] ] ) points
                           (_,(s,(x,y))) = List.foldl (\a@(size,result) r@(s,p) -> let size' = squareSize sum p s in if size' > size then (size',r) else a) (0,(0,(0,0))) squares
                       in
                         (x+1,y+1,s)

squareSize :: SumGrid -> Point -> Int -> Integer
squareSize summed p@(x,y) s = (summed ! p) + (summed ! (x + s, y + s)) - (summed ! (x, y + s)) - (summed ! (x + s, y))
