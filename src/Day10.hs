...
solution points area time = let nextPoints = (\(v, p) -> (v, v <> p)) <$> points
                                pts = snd <$> nextPoints
                                xs = fst <$> pts
                                ys = snd <$> pts
                                nextArea = (List.maximum xs - List.minimum xs) * (List.maximum ys - List.minimum ys)
                            in if nextArea >= area
                               then (Set.fromList $ snd <$> points, time)
...
renderMessage pts w h = let ps = flip (,) <$> [0..h] <*> [0..w]
                            ...
                            minX = List.minimum $ fst <$> points
                            minY = List.minimum $ snd <$> points
                            scaled = Set.map (\(x,y) -> (8 + x - minX , 2 + y - minY)) pts
                        in
                          (\p@(x,y) -> if Set.member p scaled then '\x2593' else if x == w then '\n' else '\x2591') <$> ps
